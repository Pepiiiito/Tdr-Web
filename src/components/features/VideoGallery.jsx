...
const videos = [
    ...
    {
        id: 5,
        title: 'Video: Test del Xassís (YouTube)',
        category: 'Proves',
        desc: 'Vídeo del vehicle en pista.',
        ...
        <section id="gallery" className="py-24 bg-gray-50">
                ...
                <motion.div layout className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <AnimatePresence>
                        {filteredVideos.map((video) => (
                            ...
                            >
                                <div className="relative aspect-video bg-gray-200 overflow-hidden cursor-pointer" onClick={() => setSelected(video)}>
                                    <img
                                        src={video.thumbnail}
                                        alt={video.title}
            ...
            {selected && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
                    <div className="w-full max-w-4xl bg-black rounded-md overflow-hidden">
                        <div className="relative aspect-video">
                            <iframe src={selected.youtube} title={selected.title} className="w-full h-full" allowFullScreen></iframe>
                        </div>
                        <div className="p-4 text-right">
                            <button className="px-4 py-2 bg-white text-black rounded-md" onClick={() => setSelected(null)}>Tancar</button>
                        </div>
                    </div>
                </div>
            )}
